(() => {
  ...
  navbarExpander?.addEventListener("click", (event) => {
    ...
    const navbarIsShrinked = document.body.classList.contains(
      "navbar-shrinked"
    );
  ...
  mobileNavTrigger?.addEventListener("click", (ev) => {
    ev.preventDefault();
    toggle();
    const ulEl = siteNavContainer.querySelector("ul");
    if (ulEl) {
      ulEl.removeEventListener("click", toggle);
      ulEl.addEventListener("click", toggle);
    }
  });
  bannerGradient();
  document.addEventListener("click", (ev) => {
    ...
    if (settings.contentIn) {
      if (settings.contentIn.startsWith("<") && settings.content) {
        const el = button.parentElement.closest(
