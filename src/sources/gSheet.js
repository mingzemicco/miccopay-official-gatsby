...
const SHEET_ID = process.env.SHEET_ID ?? '10VWdzJTEzmQDixlSX2nRlo-IIvVBVyKUNBj5S_kcTp4';
...
async function getConfigFromGSheet(limit) {
  ...
  if (!TotalCnt)
    throw new Error("In the first row, the value in the second column should be a valid number");
  const TitleOrderMap = {};
  ...
  if (limit)
    lastRowIdx = Math.min(2 + limit, lastRowIdx);
  ...
  for (const row of rows) {
    ...
    for (const idx in row) {
      ...
      if (key === Title.CONTENT)
        obj.content = formatContet(row[idx]);
      else if (key === Title.TAGS)
        obj.tags = row[idx].split(',');
      else
        obj[key] = row[idx];
    }
    ret.push(obj);
